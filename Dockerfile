...
#--- Clis versions
ENV ARGO_VERSION="3.6.7" \
    BBR_VERSION="1.9.74" \
    BOSH_VERSION="7.9.5" \
    BOSH_COMPLETION_VERSION="1.2.0" \
    BOSH_GEN_VERSION="0.101.2" \
    CERT_MANAGER_VERSION="2.2.0" \
    CF_VERSION="8.14.0" \
    CF_UAAC_VERSION="4.28.0" \
    CILIUM_VERSION="0.18.3" \
    CNPG_VERSION="1.25.1" \
    CREDHUB_VERSION="2.9.45" \
    CROSSPLANE_CLI="1.19.1" \
    FLUX_VERSION="2.5.1" \
    FLY_VERSION="7.11.2" \
    GITLAB_VERSION="1.57.0" \
    GITHUB_VERSION="2.72.0" \
    GOSS_VERSION="0.4.9" \
    GOVC_VERSION="0.50.0" \
    HELM_VERSION="3.17.1" \
...
ENV INIT_PACKAGES="apt-transport-https ca-certificates curl openssh-server openssl sudo unzip wget" \
    ...
    KUBECTL_PLUGINS="blame,cnpg,ctx,foreach,gadget,get-all,lineage,ns,kuttl,oidc-login,view-secret,who-can" \
    OS_ARCH_AMD="amd64" \
