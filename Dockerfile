...
ENV INIT_PACKAGES="apt-transport-https ca-certificates curl openssh-server openssl sudo unzip wget" \
    ...
    NET_PACKAGES="dnsutils iproute2 iputils-ping ldap-utils mtr-tiny netbase netcat net-tools tcpdump whois iperf3" \
...
RUN printf '\n=====================================================\n Install system packages\n=====================================================\n' && \
    ...
    printf '\n=> Add KUBECTL-CLI completion\n' && /usr/local/bin/kubectl completion bash > /etc/bash_completion.d/kubectl && /usr/local/bin/kubectl completion bash | sed -e "s+kubectl+k+g" > /etc/bash_completion.d/k && \
