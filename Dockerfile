...
#--- Update the image and install missing and tools packages
...
#--- Add go binary to default PATH
ADD scripts/go.sh /etc/profile.d/
RUN chmod 755 /etc/profile.d/go.sh

#--- Setup SSH access & secure root login (SSH login fix. Otherwise user is kicked off after login) & logs (/var/log/auth.log)
...
RUN chmod 755 /etc/profile.d/homedir.sh /etc/profile.d/cf.sh && \
    ...
    echo "" >> /home/${container_login}/.profile && \
    echo "# Persistant user configuration (over the shared disk)" >> /home/${container_login}/.profile && \
    echo "if [ -f \"/home/bosh/shared/\`hostname\`/.bashrc\" ] ; then" >> /home/${container_login}/.profile && \
    echo "  . "/home/bosh/shared/\`hostname\`/.bashrc"" >> /home/${container_login}/.profile && \
    echo "fi" >> /home/${container_login}/.profile

#--- Install bosh-init, spiff, spiff++, spruce, bosh2, cf-cli, terraform, plugin terraform_provider_cloudfoundry, fly, credhub, gof3r, certstrap & several cf cli plugins
RUN wget -nv -O /usr/local/bin/bosh-init "https://s3.amazonaws.com/bosh-init-artifacts/bosh-init-${bosh_init_version}-linux-amd64" && \
    chmod 755 /usr/local/bin/bosh-init && \
    wget -nv -O /tmp/spiff_linux_amd64.zip "https://github.com/cloudfoundry-incubator/spiff/releases/download/v${spiff_version}/spiff_linux_amd64.zip" && \
    unzip -q /tmp/spiff_linux_amd64.zip -d /usr/local/bin && \
    chmod 755 /usr/local/bin/spiff && \
    rm /tmp/spiff_linux_amd64.zip && \
    wget -nv -O /tmp/spiff_linux_amd64.zip "https://github.com/mandelsoft/spiff/releases/download/v${spiff_reloaded_version}/spiff_linux_amd64.zip" && \
    unzip -q /tmp/spiff_linux_amd64.zip -d /usr/local/bin && \
    chmod 755 /usr/local/bin/spiff++ && \
    rm /tmp/spiff_linux_amd64.zip && \
    wget -nv -O /usr/local/bin/spruce "https://github.com/geofffranks/spruce/releases/download/v${spruce_version}/spruce-linux-amd64" && \
    chmod 755 /usr/local/bin/spruce && \
    wget -nv -O /usr/local/bin/bosh2 "https://s3.amazonaws.com/bosh-cli-artifacts/bosh-cli-${bosh_cli_v2_version}-linux-amd64" && \
    chmod 755 /usr/local/bin/bosh2 && \
    wget -nv -O /tmp/cf.deb "https://cli.run.pivotal.io/stable?release=debian64&version=${cf_cli_version}&source=github-rel" && \
    dpkg -i /tmp/cf.deb && \
    rm /tmp/cf.deb && \
    wget -nv -O /tmp/terraform.zip "https://releases.hashicorp.com/terraform/${terraform_version}/terraform_${terraform_version}_linux_amd64.zip" && \
    ...
    wget -nv -O /usr/local/bin/fly "https://github.com/concourse/concourse/releases/download/v${fly_version}/fly_linux_amd64" && \
    chmod 755 /usr/local/bin/fly && \
    wget -nv -O /tmp/credhub-linux.tgz "https://github.com/cloudfoundry-incubator/credhub-cli/releases/download/${credhub_version}/credhub-linux-${credhub_version}.tgz" && \
    tar -xzvf /tmp/credhub-linux.tgz -C /usr/local/bin && \
    chmod 755 /usr/local/bin/credhub && \
    rm /tmp/credhub-linux.tgz && \
    GO15VENDOREXPERIMENT=1 && \
    export GOPATH=/tmp && \
    go get github.com/rlmcpherson/s3gof3r/gof3r && \
    mv /tmp/bin/gof3r /usr/local/bin/gof3r && \
    chmod 755 /usr/local/bin/gof3r && \
    rm -fr /tmp/* && \
    su -c "export http_proxy=${http_proxy};export https_proxy=${https_proxy};go get -v github.com/square/certstrap" -l ${container_login} && \
    cp /home/${container_login}/go/bin/certstrap /usr/local/bin/certstrap && \
    chmod 755 /usr/local/bin/certstrap && \
    rm -fr /home/${container_login}/go && \
    wget -nv -O /usr/local/bin/jq "https://github.com/stedolan/jq/releases/download/jq-1.5/jq-linux64" && \
...
#--- Tools publication on system banner, final settings & Cleanup
ADD scripts/motd /etc/
RUN GIT_VERSION=`git --version | awk '{print $3}'` && \
    CERTSTRAP_VERSION=`/home/bosh/go/bin/certstrap -v | awk '{print $3}'` && \
