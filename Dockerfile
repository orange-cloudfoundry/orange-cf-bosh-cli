...
#--- Clis versions
ENV ARGO_VERSION="4.0.0" \
    BBR_VERSION="1.9.76" \
    BOSH_VERSION="7.9.18" \
    BOSH_COMPLETION_VERSION="1.2.0" \
    BOSH_GEN_VERSION="0.101.2" \
    CERT_MANAGER_VERSION="2.4.0" \
    CF_VERSION="8.17.1" \
    CF_UAAC_VERSION="4.29.0" \
    CILIUM_VERSION="0.19.0" \
    CNPG_VERSION="1.28.1" \
    CREDHUB_VERSION="2.9.54" \
    CROSSPLANE_CLI="2.1.4" \
    FLUX_VERSION="2.7.5" \
    FLY_VERSION="7.14.3" \
    GITLAB_VERSION="1.85.2" \
    GITHUB_VERSION="2.86.0" \
    GOSS_VERSION="0.4.9" \
    GOVC_VERSION="0.52.0" \
    HELM_VERSION="3.17.1" \
    HUBBLE_VERSION="1.18.6" \
    JQ_VERSION="1.8.1" \
    JWT_VERSION="6.2.0" \
    KCTRL_VERSION="0.59.1" \
    KLBD_VERSION="0.47.1" \
    KREW_VERSION="0.4.5" \
    KUBECTL_VERSION="1.32.12" \
...
ENV INIT_PACKAGES="apt-transport-https ca-certificates curl openssh-server openssl sudo unzip wget" \
    ...
    KUBECTL_PLUGINS="blame,cnpg,ctx,foreach,gadget,get-all,lineage,ns,kuttl,oidc-login,resource-capacity,view-secret,who-can" \
    OS_ARCH_AMD="amd64" \
...
RUN printf '\n=====================================================\n Install system packages\n=====================================================\n' && \
    ...
    printf '\n=> Add MYSQL-TOOLS\n' && curl -sSLo /tmp/mysql-tools.deb "https://dev.mysql.com/get/Downloads/MySQL-Workbench/mysql-workbench-community_${MYSQL_VERSION}-1ubuntu22.04_${OS_ARCH_AMD}.deb" && dpkg --fsys-tarfile /tmp/mysql-tools.deb | tar -xO ./usr/lib/mysql-workbench/mysql > /usr/local/bin/mysql && dpkg --fsys-tarfile /tmp/mysql-tools.deb | tar -xO ./usr/lib/mysql-workbench/mysqldump > /usr/local/bin/mysqldump && \
