...
display "bosh-depls-failed" "Get bosh deployments with failed status for current bosh-director"
...
display "git-filter-repo" "Tool to squash git history on repositories"
...
printf "\n%bGENERIC TOOLS%b\n" "${GREEN}${BOLD}${REVERSE}" "${STD}"
display "bosh-depls-failed" "Get bosh failed deployments instances"
