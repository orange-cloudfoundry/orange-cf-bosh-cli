...
if [ "${bosh_properties}" != "null" ] ; then
  ...
  key="$(echo "${tags}" | jq -r '.|select(.name == "env_type")|.key')"
  env_type="$(echo "${bosh_properties}" | jq -r --arg KEY "${key}" '.[]|select(.key|tostring == $KEY)|.value')"
  ...
  printf "bosh environment        : ${env_type}\n"
